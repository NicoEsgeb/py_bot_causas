...
def fill_form_escritos(row, escritos_DCP_Path: str):
    ...
    ruta_escrito_operacion += "\\Trib_" + str(row["TRIBUNAL"])[:-1] + "°_ROL_" + str(row["ROL"]) + "_"
    #ruta_escrito_operacion += "\\Trib_" + str(row["TRIBU"]) + "_ROL_" + str(row["ROL"]) + "_"
    format_rut = int(row["RUT"])
    format_rut = f"{format_rut:,}".replace(",", ".") 
    ruta_escrito_operacion += format_rut + "-" + str(row["DV"]) + ".pdf" 
